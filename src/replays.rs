    ...
    match tokio::fs::read_dir(&replays_dir).await {
        Ok(mut entries) => {
            while let Ok(Some(entry)) = entries.next_entry().await {
                ...
                // Using eq_ignore_ascii_case instead of to_lowercase() eliminates O(N) double-string
                // heap allocations when iterating through the directory to find a matching file.
                if entry
                    .file_name()
                    .to_str()
