    ...
    match tokio::fs::read_dir(&replays_dir).await {
        Ok(mut entries) => {
            while let Ok(Some(entry)) = entries.next_entry().await {
                ...
                // 💡 What: Replaced `.to_lowercase() == Some(...to_lowercase())` with `.is_some_and(|s| s.eq_ignore_ascii_case())`
                // 🎯 Why: Avoids two O(N) heap allocations (String creations) per file in the directory scan
                // 📊 Impact: Significantly reduces memory pressure and CPU time when scanning directories with many replays
                if entry
                    .file_name()
                    .to_str()
