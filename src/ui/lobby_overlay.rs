...
pub(super) fn rank_icon(rank: &str) -> Option<egui::ImageSource<'static>> {
    let r = rank.trim();
    // OPTIMIZATION: Replacing `.to_lowercase()` with `.eq_ignore_ascii_case()` avoids creating a new
    // String allocation on the heap for every rank comparison, which is important since this function
    // might be called frequently during render loops.
    // EXPECTED IMPACT: Eliminates 1 heap allocation per player, per frame (when rendering lobby icons).
