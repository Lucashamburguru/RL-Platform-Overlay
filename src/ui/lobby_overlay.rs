...
pub(super) fn rank_icon(rank: &str) -> Option<egui::ImageSource<'static>> {
    // Optimization: Avoid calling `.to_lowercase()` which allocates a new String on the heap
    // every frame. We use `eq_ignore_ascii_case` instead for zero-allocation string comparison.
    let r = rank.trim();
