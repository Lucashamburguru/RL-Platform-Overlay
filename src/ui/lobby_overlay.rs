...
// Provides a zero-allocation, case-insensitive substring search to avoid
...
pub(super) fn render_platform_name(ui: &mut egui::Ui, platform: &str, size: f32) {
    ...
    if contains_ignore_ascii_case_zero_alloc(normalized, "epic") {
...
pub(super) fn platform_icon_for(platform: &str, is_bot: bool) -> egui::ImageSource<'static> {
    ...
    if contains_ignore_ascii_case_zero_alloc(platform, "steam") {
