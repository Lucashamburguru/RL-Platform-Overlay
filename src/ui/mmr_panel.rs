...
// Optimization: A zero-allocation implementation of case-insensitive substring search.
// We avoid allocating a new String via `to_lowercase()` every frame by inspecting byte windows.
...
fn ranked_playlist_sort_priority(playlist_id: i32, playlist_name: &str) -> i32 {
    if playlist_id == 10
        || contains_ignore_case(playlist_name, "duel")
        || contains_ignore_case(playlist_name, "1v1")
    {
        0
    } else if playlist_id == 11
        || contains_ignore_case(playlist_name, "doubles")
        || contains_ignore_case(playlist_name, "2v2")
    {
        1
    } else if playlist_id == 13
        || contains_ignore_case(playlist_name, "standard")
        || contains_ignore_case(playlist_name, "3v3")
    {
        2
...
fn compact_playlist_name(playlist_name: &str) -> String {
    if contains_ignore_case(playlist_name, "duel") || contains_ignore_case(playlist_name, "1v1") {
        "1v1".to_string()
    } else if contains_ignore_case(playlist_name, "doubles")
        || contains_ignore_case(playlist_name, "2v2")
    {
        "2v2".to_string()
    } else if contains_ignore_case(playlist_name, "standard")
        || contains_ignore_case(playlist_name, "3v3")
    {
        "3v3".to_string()
    } else if contains_ignore_case(playlist_name, "unranked")
        || contains_ignore_case(playlist_name, "un-ranked")
        || contains_ignore_case(playlist_name, "casual")
    {
        "Casual".to_string()
