...
fn ranked_playlist_sort_priority(playlist_id: i32, playlist_name: &str) -> i32 {
    use crate::ui::common::contains_ignore_ascii_case as contains;
    if playlist_id == 10 || contains(playlist_name, "duel") || contains(playlist_name, "1v1") {
        0
    } else if playlist_id == 11
        || contains(playlist_name, "doubles")
        || contains(playlist_name, "2v2")
    {
        1
    } else if playlist_id == 13
        || contains(playlist_name, "standard")
        || contains(playlist_name, "3v3")
    {
        2
    } else if playlist_id == 0
        || contains(playlist_name, "unranked")
        || contains(playlist_name, "un-ranked")
        || contains(playlist_name, "casual")
    {
        3
    } else {
        10
...
fn compact_playlist_name(playlist_name: &str) -> String {
    use crate::ui::common::contains_ignore_ascii_case as contains;
    if contains(playlist_name, "duel") || contains(playlist_name, "1v1") {
        "1v1".to_string()
    } else if contains(playlist_name, "doubles") || contains(playlist_name, "2v2") {
        "2v2".to_string()
    } else if contains(playlist_name, "standard") || contains(playlist_name, "3v3") {
        "3v3".to_string()
    } else if contains(playlist_name, "unranked")
        || contains(playlist_name, "un-ranked")
        || contains(playlist_name, "casual")
    {
        "Casual".to_string()
