...
pub fn parse_platform(id: &str) -> (String, bool) {
    ...
    if id.eq_ignore_ascii_case("Unknown|0|0") {
        return ("BOT".to_string(), true);
    }
    // Avoid collecting split parts into a Vec
    let platform = id.split('|').next().unwrap_or("Unknown");

    // Use eq_ignore_ascii_case to avoid allocating a new String with to_lowercase
...
pub fn format_platform(platform: &str) -> &str {
    // Avoid allocating string in hot path by using eq_ignore_ascii_case
